...
class QuizWizardApp {
    ...
    private handleAction(action: string) {
        ...
        switch (action) {
            ...
            case 'ex-change-scope': this.setExamScope(); break;
            case 'ex-save-paper':   this.saveExamPaper(); break;

            /* --- Student List (학생 명단) --- */
            case 'sl-new':          this.newStudentList(); break;
            case 'sl-open':         this.openStudentList(); break;
            case 'sl-edit':         this.editStudentList(); this.initializeStudentListWorkspace(); break;
            case 'sl-save':         this.saveStudentList(); this.initializeStudentListWorkspace(); break;
            case 'sl-save-as':      this.saveAsStudentList(); this.initializeStudentListWorkspace(); break;
    ...
    private initializeQuestionBankWorkspace(forceReset: boolean = false, skipSave: boolean = false) {
        ...
        if (forceReset || this.questionsData.length === 0) {
            ...
            if (forceReset) this.question_bank_file_name = ''; // 새 파일이므로 이름 초기화
        }
    ...
    private setLanguage() {
        ...
        const currentRadio = dialog.querySelector(`input[value="${this.currentLang}"]`) as HTMLInputElement;
        if (currentRadio) {
            currentRadio.checked = true;
        }
