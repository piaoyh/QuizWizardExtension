...
class QuizWizardApp {
    ...
    /** 학생 명단 작업공간 초기화 */
    private initializeStudentListWorkspace(forceReset: boolean = false) {
    ...
    private initializeSelfStudyWorkspace() {
        ...
        // 메뉴 강조
        document.querySelectorAll('.menu-item').forEach(btn => {
            if ((btn as HTMLElement).dataset.menu === 'self-study') {
                btn.classList.add('active');
            } else {
                btn.classList.remove('active');
            }
    ...
    /** 하나의 문제 항목 HTML 생성 */
    private createQuestionItemHtml(index: number, data?: QuestionState, readOnly: boolean = false, hideGroup: boolean = false, checkDisabled: boolean = true): string {
    ...
    private async openQuestionBank()
    {
        try {
            ...
            if (err.name !== 'AbortError') console.error("파일 접근 오류:", err);
        }
    }
