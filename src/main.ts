...
class QuizWizardApp {
    ...
    private init() {
        ...
        submenuItems.forEach(item => {
            item.addEventListener('click', (e) => {
                ...
                // 부모 메뉴 찾기
                const parentBtn = target.closest('.menu-item-container')?.querySelector('.menu-item') as HTMLElement;
    ...
    private handleAction(menu: string, action: string) {
        // 하위 메뉴 클릭 시 동작 정의
        if (action === 'theme') {
    ...
    private renderView(menu: string) {
        if (!this.container || this.currentMenu === menu) return;
