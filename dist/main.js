...
class QuizWizApp {
    ...
    default_translations = null; // 기본 영어 스트링 저장용
    ...
    updateDialogLanguage() {
        ...
        if (langTitle)
            langTitle.textContent = langData.actions['st-lang'] || defaultLangData.actions['st-lang'];
        const themeTitle = document.getElementById('theme-dialog-title');
        if (themeTitle)
            themeTitle.textContent = langData.actions['st-theme'] || defaultLangData.actions['st-theme'];
        ...
        if (fontDialogTitle)
            fontDialogTitle.textContent = langData.actions['st-font-dialog-title'] || defaultLangData.actions['st-font-dialog-title'];
        if (fontLabel)
            fontLabel.textContent = langData.actions['st-font-label'] || defaultLangData.actions['st-font-label'];
        if (fontGuide)
            fontGuide.textContent = langData.actions['st-font-guide'] || defaultLangData.actions['st-font-guide'];
        const submitTitle = document.getElementById('submit-dialog-title');
        if (submitTitle)
            submitTitle.textContent = langData.actions['ss-submit-paper'] || defaultLangData.actions['ss-submit-paper'];
        const scopeTitle = document.getElementById('scope-dialog-title');
        if (scopeTitle)
            scopeTitle.textContent = langData.actions['st-scope-title'] || defaultLangData.actions['st-scope-title'];
        const scopeGuide = document.getElementById('scope-guide');
        if (scopeGuide)
            scopeGuide.textContent = langData.actions['st-scope-guide'] || defaultLangData.actions['st-scope-guide'];
        const scopeCountLabel = document.getElementById('scope-count-label');
        if (scopeCountLabel)
            scopeCountLabel.textContent = langData.actions['st-scope-count-label'] || defaultLangData.actions['st-scope-count-label'];
        ...
        document.querySelectorAll('.modal-actions button').forEach(btn => {
            if (btn.id.includes('confirm'))
                btn.textContent = langData.actions['dlgbx-button-ok'] || defaultLangData.actions['dlgbx-button-ok'];
            if (btn.id.includes('cancel'))
                btn.textContent = langData.actions['dlgbx-button-cancel'] || defaultLangData.actions['dlgbx-button-cancel'];
        });
    }
    bindEvents() {
        // 주메뉴 클릭 핸들러
                ...
                switch (e.key.toUpperCase()) {
                    ...
                    case '1':
                        if (!e.shiftKey) {
                            ...
                            if (isActionDisabled('ex-save-paper-docx'))
                                return;
                    ...
                    case '2':
                        if (!e.shiftKey) {
                            ...
                            if (isActionDisabled('ex-save-paper-pdf'))
                                return;
                    ...
                    case '3':
                        if (!e.shiftKey) {
                            ...
                            if (isActionDisabled('ex-save-paper-txt'))
                                return;
    ...
    handleAction(action, value) {
        ...
        switch (action) {
            ...
            case 'ex-save-paper':
                this.saveExamPaper();
                break;
            case 'ex-save-paper-docx':
                this.saveExamPaper('docx');
                break;
            case 'ex-save-paper-pdf':
                this.saveExamPaper('pdf');
                break;
            case 'ex-save-paper-txt':
                this.saveExamPaper('txt');
                break;
            /* --- Student List (학생 명단) --- */
            case 'sl-new':
    ...
    async applyDefaultHeaderContent(lang) {
        ...
        if (nameInput)
            nameInput.value = langData.actions['ph-header-name'] || defaultLangData.actions['ph-header-name'];
        if (idInput)
            idInput.value = langData.actions['ph-header-id'] || defaultLangData.actions['ph-header-id'];
        if (noticeInput) {
    ...
    newQuestionBank() {
        ...
        if (titleEl)
            titleEl.textContent = langData.actions['qb-new-confirm-title'] || defaultLangData.actions['qb-new-confirm-title'];
        if (msgEl)
            msgEl.textContent = langData.actions['qb-new-confirm-msg'] || defaultLangData.actions['qb-new-confirm-msg'];
        if (yesBtn)
            yesBtn.textContent = langData.actions['qb-new-yes'] || defaultLangData.actions['qb-new-yes'];
        if (noBtn)
            noBtn.textContent = langData.actions['qb-new-no'] || defaultLangData.actions['qb-new-no'];
    ...
    statQuestionBank() {
        ...
        const categoryLength = Object.keys(categoryCounts).length;
    ...
    newStudentList() {
        ...
        if (titleEl)
            titleEl.textContent = langData.actions['sl-confirm-new-title'] || defaultLangData.actions['sl-confirm-new-title'];
        if (msgEl)
            msgEl.textContent = langData.actions['sl-confirm-new-msg'] || defaultLangData.actions['sl-confirm-new-msg'];
        if (yesBtn)
            yesBtn.textContent = langData.actions['sl-confirm-new-yes'] || defaultLangData.actions['sl-confirm-new-yes'];
        if (noBtn)
            noBtn.textContent = langData.actions['sl-confirm-new-no'] || defaultLangData.actions['sl-confirm-new-no'];
        ...
        try {
            ...
            if (fixedType) {
                types = allTypes.filter(t => {
                    const extensions = Object.values(t.accept)[0];
